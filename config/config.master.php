 ...
 *
 * require FCPATH . '/config/config.master.php';
 *
...
if (isset($config))
{
    ...
    $base_url                          = $protocol . $_SERVER['HTTP_HOST'] . rtrim(substr($_SERVER["PHP_SELF"], 0, -strlen(SELF)), '\\/');
    $base_path                         = FCPATH;
     ...
     * This array will not create new upload entries, a db match is a must,
     * see http://expressionengine.com/user_guide/cp/content/files/file_upload_preferences.html#overriding-upload-paths-and-urls-using-configuration-variables
     */
    $env_config['upload_preferences'] = array(
        1   => array(
            'name'          => 'Default upload',
            'server_path'   => $base_path . 'public/upload/default/',
            'url'           => $base_url  . '/public/upload/default/'
        ),
        2   => array(
            'name'          => 'Book covers',
            'server_path'   => $base_path . 'public/upload/books/covers/',
            'url'           => $base_url  . '/public/upload/books/covers/'
        ),
        3   => array(
            'name'          => 'Movie posters',
            'server_path'   => $base_path . 'public/upload/movies/posters/',
            'url'           => $base_url  . '/public/upload/movies/posters/'
        ),
    );
    ...
    // stash config additions
    $env_config['stash_file_basepath'] = str_replace('\\', '/', $env_config['tmpl_file_basepath'] . '/default_site/');
   ...
   $env_config['gzip_output']                 = 'y';
     ...
     * @see config/config.prod.php
     */
    require FCPATH . '/config/config.' . ENV . '.php';
